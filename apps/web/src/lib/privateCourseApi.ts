...
function asJobs(value: unknown) {
  if (!Array.isArray(value)) throw new Error('任务列表响应无效')
  return value as ProcessingJob[]
}
...
function asTranslationResponse(value: unknown) {
  if (!value || typeof value !== 'object' || !('coverage' in value) || !('vocabularyCoverage' in value)) throw new Error('璇剧▼缈昏瘧鍝嶅簲鏃犳晥')
...
export function getProcessingJob(jobId: string, signal?: AbortSignal) {
  return request(`/me/jobs/${jobId}`, { signal }, asJob)
}

export function listProcessingJobs() {
  return request('/me/jobs', {}, asJobs)
}
...
export function translatePrivateCourse(id: string) {
  return request(`/me/courses/${id}/translate`, { method: 'POST', body: '{}' }, asTranslationResponse)
}

export type { PrivateCourseSummary, PrivateLessonDetail, ProcessingJob, TranslateCourseResponse }

...
function transcriptCopy(asset: MediaAssetView) {
  ...
  if (state?.status === 'succeeded' && state.stage === 'transcript_ready') {
    return { tone: 'ready', label: '字幕已准备', detail: '完整英文逐词字幕已经原子发布', retryable: false }
  }
  if (asset.durationMs === null) return {
    tone: 'failed', label: '字幕不支持',
    detail: '原片仍可播放 · 无法确认视频时长，不能生成或重试字幕', retryable: false,
  }
  const durationUnsupported = asset.durationMs > MAX_UPLOAD_DURATION_MS || state?.errorCode === 'media_duration_unsupported'
  if (durationUnsupported) return {
    tone: 'failed', label: '字幕不支持',
    detail: '原片仍可播放 · 超过 60 分钟的视频不能生成或重试字幕', retryable: false,
  }
  if (!state) return { tone: 'waiting', label: '等待字幕任务', detail: '原片可播放，字幕链尚未启动', retryable: false }
  if (state.status === 'failed' || state.status === 'cancelled') {
    return { tone: 'failed', label: '字幕生成失败', detail: `原片仍可播放 · ${state.errorCode ?? '请稍后重试'}`, retryable: true }
  }
  const chunks = state.totalChunks ? ` · ${state.completedChunks}/${state.totalChunks} 个分片` : ''
  return { tone: 'processing', label: '正在生成字幕', detail: `${stageCopy[state.stage ?? ''] ?? '正在准备字幕'}${chunks}`, retryable: false }
}

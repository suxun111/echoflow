...
import { useLearningStreakTimeSync } from '../hooks/useLearningStreakTimeSync'
...
import { LearningStreakLabel } from './LearningStreak'
...
export function PlatformShell({ children, view, search, onSearch, onView, onUpload }: { children: ReactNode; view: AppView; search: string; onSearch: (value: string) => void; onView: (view: AppView) => void; onUpload: () => void }) {
  ...
  useLearningStreakTimeSync()

  return <div className="platform-shell"><aside className="sidebar"><Logo/><nav className="side-nav" aria-label="主导航">{navigation.map((item) => <button key={item.id} className={view === item.id ? 'active' : ''} onClick={() => onView(item.id)}><Icon name={item.icon}/><span>{item.label}</span>{item.id === 'vocabulary' && vocabularyCount > 0 && <b>{vocabularyCount}</b>}</button>)}</nav><div className="side-divider"/><p className="side-label">我的空间</p><button className={`side-upload ${view === 'uploads' ? 'active' : ''}`} onClick={() => onView('uploads')}><Icon name="upload"/><span>个人上传</span><small>AI 字幕</small></button><DailyGoalCard onOpen={() => onView('history')}/><button className="profile-button" onClick={() => onView('settings')}><span className="avatar">L</span><span><strong>Lena</strong><LearningStreakLabel/></span><Icon name="settings" size={17}/></button></aside>

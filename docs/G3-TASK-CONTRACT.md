...
对 G2 已验证、可播放的私人长视频，系统从原始媒体中提取规范化英文语音，按可恢复的持久任务切片提交给 MOSS，并在所有必需分片均成功、逐词时间戳通过完整性校验后，一次性发布唯一 ACTIVE 的英文字幕版本。
...
- 单个分片成功不等于字幕成功；只有完整、单调、无越界且含逐词时间戳的全部英文结果才允许发布；
...
- 默认目标分片约 10 分钟，在静音附近择边界并保留小幅上下文重叠；每片保存绝对 `startMs/endMs`，合并时基于绝对时间去重；
...
- 逐词结构至少包含 `text/startMs/endMs`；所有数值使用毫秒整数，不使用浮点秒作为数据库事实；
- 分片偏移恢复后，依据重叠区词序与时间做确定性去重；不能只把数组首尾拼接；
- 校验覆盖：每个必需分片成功、索引连续、词文本非空、`0 ≤ startMs < endMs ≤ durationMs`、全局时间单调、Cue 不倒序、没有明显大段未覆盖；
- Cue 分段只基于已校验的英文词序和标点/停顿；不生成中文翻译、说话人审核或人工字幕；
...
- 结果为空、缺词时间戳、时间倒退、越界、重复重叠、缺片与损坏 checksum；
- 发布事务失败时旧 ACTIVE 不变；重复发布仍只有一个 ACTIVE 版本。

### 6.2 真实媒体与真实 MOSS

- 30 秒英文黄金样本用于日常真实集成，核对逐词时间与可读文本；
- 5、30、60、120 分钟英文样本各至少一次完整链路，核对分片、重叠去重、单调时间、最终 Cue 数与播放总时长；
...
4. 30 秒及 5/30/60/120 分钟真实 MOSS 样本矩阵通过；
5. 120 分钟结果具备完整英文逐词时间戳，全局单调且不越界；

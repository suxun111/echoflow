...
冻结日期：2026-08-13；segment-first 修订确认：2026-08-15
基线：`master@0c3e2ee`  
...
对 G2 已验证、可播放的私人长视频，系统从原始媒体中提取规范化英文语音，按可恢复的持久任务切片提交给 MOSS，并在所有必需分片均成功、英文分段及时间边界通过完整性校验后，一次性发布唯一 ACTIVE 的英文字幕版本。逐词时间戳是可选增强，不是 V1 发布前提。
...
5. 120 分钟结果具备完整英文分段和真实起止时间，全局单调且不越界；逐词时间戳若存在也必须通过相同校验；

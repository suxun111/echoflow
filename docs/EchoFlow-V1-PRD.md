...
amended: 2026-08-24（G3 交接证据分层确认；V1 时长上限收敛为 60 分钟）
supersedes:
...
- 私人长视频上传；
- MP4 / H.264 / AAC；
- 私人英语 MP4 的产品时长 `0 < durationMs ≤ 3_600_000`（最长 60 分钟，含 60:00）；单文件大小上限 8 GiB；
- 原始清晰度保存；
- 上传完成后真实播放；
- MOSS 自动生成完整英文字幕；
- 5–10 分钟学习单元；
...
V1 的个人上传与字幕处理时长上限由 120 分钟收敛为 **≤60 分钟**。浏览器可以在上传前读取元数据并提示；Worker 对对象进行 ffprobe/媒体探测后才是最终事实源。超过上限或无法确认真实时长的媒体必须失败、不得成为 `PLAYABLE`，也不得进入 G3 字幕处理；8 GiB 仍是独立的文件大小安全上限，不能替代时长判断。

本修订适用于后续产品准入、实现和 Gate 验收；不追溯改写已经发生的 G2/G3 历史测试、失败关闭或 Git 证据。
...
合规格式以 UI 和服务端统一契约为准：MP4、H.264、AAC；V1 稳定验收 5、30、60 分钟样本。`0 < durationMs ≤ 3_600_000` 是时长准入，单文件 8 GiB 是独立的字节准入；浏览器预检不可替代 Worker 的真实 ffprobe/媒体探测，超 60 分钟或无法确认时长的输入不得成为 `PLAYABLE` 或进入 G3。
...
- MOSS 至少需要返回完整英文分段及每段准确的起止时间；词级时间戳是可选增强；但切片输入的发布前提是存在可验证的跨分片唯一交接证据。段级结果只有在严格合并实际得到该证据时才足够；否则必须失败关闭，或由 Provider/经过验证的对齐层提供真实补充证据；
- V1 允许一个经过校验的真实 MOSS 分段直接映射为一个 Cue；没有声学证据时不得按字符数或平均时长伪造单词时间；
- 只有整篇文本而无分段时间边界的模型不满足影子学习要求。
...
建议生成 16 kHz、单声道规范化音频；保持视频零点，不裁掉开头静音。长音频的目标分片时长、静音边界与重叠必须由真实 MOSS 的模型、硬件和交接基准共同决定，并持久化为计划事实；不得把某个设备上的“约 10 分钟”当作默认承诺。
...
在购买常驻 GPU 前，用 10、30、60 分钟真实样本测试冷启动、预热、并发 1/2，并记录：RTF、峰值 VRAM/RAM、时间戳误差、错误率、OOM 和单音频小时成本。这是 V1 ≤60 分钟范围内的容量与成本基准，不替代 G3 的完整字幕发布矩阵。

G3 的真实证据按用途分层：G3-A 证明 30 秒模型/Provider 与最小完整发布链路；G3-B 先以新的受权、可重复英语样本证明跨分片唯一交接路线；G3-C 再完成 5/30/60 分钟完整发布、故障恢复与安全矩阵。三个阶段均不降低 60 分钟完整字幕的最终门禁；详细合同见 [G3-B 分片交接能力基准合同](G3-B-SEGMENT-HANDOFF-BENCHMARK-CONTRACT.md)。
...
- 60 分钟黄金链路连续多轮；
- 8 GiB 的字节、整数和 Multipart Part 上限验证；
...
已冻结：单文件 8 GiB、时长 `≤60 分钟`、每人一个活动上传、每用户默认 20 GB。
...
退出条件：5/30/60 分钟样本通过；断网、刷新、签名过期、重复 complete、API 崩溃均不丢失或重复资产。

### G3：MOSS 和完整英文字幕

G3 内部依次形成 G3-A 基础真实链路、G3-B 跨分片交接可行性决策、G3-C 长媒体可靠性与容量证据；这只是组织证据的顺序，不构成新 Gate，不解冻 G4，也不降低以下交付或退出条件。
...
退出条件：真实 60 分钟视频产生完整、单调、无越界、带准确分段起止时间的英文字幕；若 Provider 返回逐词时间则逐词时间也必须真实、单调且无越界，任一分片失败不会发布半套字幕。
...
> 一名受邀目标用户能够上传一段最长 60 分钟（含 60:00）的 MP4 英语视频；上传中断后在 7 天内继续；上传完成后原始清晰度视频可播放；MOSS 在后台生成唯一完整且时间准确的英文字幕；用户能在 5–10 分钟学习单元中逐句循环、录音、回听，并在第二次打开时准确继续；全流程在桌面和手机可完成，没有假进度、死按钮、跨用户访问、不可恢复数据或已知 P0/P1 Bug。
...
7. 再扩展到 5、30、60 分钟，并加入断网、重启和重复投递；
8. 接入真实 MOSS，完成 10/30/60 分钟基准；
...
G3-A：已有真实基础链路证据
→ G3-B：先证明跨分片交接路线
→ G3-C：5/30/60 完整发布与故障恢复
→ 10/30/60 容量与采购基准
→ 仍以既有 60 分钟完整字幕等 Gate 条件关闭 G3
```

不得用 G3-A、G3-B 的局部完成进入 G4，也不得因为所有 MOSS chunk 成功就声明字幕发布成功。

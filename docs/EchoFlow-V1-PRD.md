...
amended: 2026-08-23（G3 交接证据分层确认）
supersedes:
...
在购买常驻 GPU 前，用 10、60、180 分钟真实样本测试冷启动、预热、并发 1/2，并记录：RTF、峰值 VRAM/RAM、时间戳误差、错误率、OOM 和单音频小时成本。这是容量、成本与 3 小时架构上限的采购基准，不替代 G3 的完整字幕发布矩阵。

G3 的真实证据按用途分层：G3-A 证明 30 秒模型/Provider 与最小完整发布链路；G3-B 先以新的受权、可重复英语样本证明跨分片唯一交接路线；G3-C 再完成 5/30/60/120 分钟完整发布、故障恢复与安全矩阵。三个阶段均不降低 120 分钟完整字幕的最终门禁；详细合同见 [G3-B 分片交接能力基准合同](G3-B-SEGMENT-HANDOFF-BENCHMARK-CONTRACT.md)。
...
退出条件：真实 120 分钟视频产生完整、单调、无越界、带准确分段起止时间的英文字幕；若 Provider 返回逐词时间则逐词时间也必须真实、单调且无越界，任一分片失败不会发布半套字幕。
...
G3-A：已有真实基础链路证据
→ G3-B：先证明跨分片交接路线
→ G3-C：5/30/60/120 完整发布与故障恢复
→ 10/60/180 容量与采购基准
→ 仍以既有 120 分钟完整字幕等 Gate 条件关闭 G3
```

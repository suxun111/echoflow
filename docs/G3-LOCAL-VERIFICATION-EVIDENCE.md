...
本地代码验收锚点：`805b93fb1f39c55bc75e793e988a75b431cb2de9`

状态：本地实现候选通过；G3 Gate 保持打开

## 1. 结论

`8139081` 已形成可恢复的本地 G3 候选链路：从已 PLAYABLE 的私人 MP4 提取 16 kHz 单声道音频、静音附近分片、通过统一 Adapter 执行 MOSS `submit/query/result/cancel`、接收 HMAC 回调并以轮询兜底、保存不可变 ASR 原始结果、合并和校验逐词时间戳，最后在单个数据库事务中发布唯一 ACTIVE 英文字幕。`805b93f` 在真实浏览器上传复验中进一步消除了播放处理与字幕处理的状态混淆。
...
- API 只把 `g3-transcript-v1` ProcessingRun 序列化为字幕进度；Web 只在持久阶段确为 `transcript_ready` 时显示“字幕已完成”，不用 G2 播放 Run 或假百分比冒充字幕进度。

## 3. 代码锚点 `805b93f` 的确定性验证
...
| `pnpm test` | 148 passed，4 skipped | 包含真实 PostgreSQL/MinIO/Redis/FFmpeg 和确定性 Fake MOSS；跳过项是需设置 `RUN_G2_LONG_MEDIA=true` 才执行的自生成 G2 长媒体矩阵 |
...
最终对核心流水线 `8139081` 的结论为：未发现已知 P0/P1。浏览器复验发现 G2 播放 Run 会被误显示成“字幕已完成”后，`805b93f` 分离 G2/G3 查询与展示语义并增加 API/Web 回归测试；独立 Reviewer 对该增量同样未发现 P0/P1，批准当前本地代码锚点。Reviewer 另记录一项非阻断 P2：后续应拆分 G2 媒体诊断与 G3 字幕查询，以保留 `media_format_unsupported` 等专用错误，并补 G2/G3 Run 并存的正向 E2E。审查不包含真实 MOSS 运行验证。

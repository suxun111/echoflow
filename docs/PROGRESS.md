...
验证证据已提交为 `3514a4e` 并进入唯一 trunk，G2 的 8 项退出条件全部完成。G2 的绿色不代表字幕或学习闭环已完成。

## G3 已开始

- 用户已确认开始“MOSS 与完整字幕原子发布”，实施分支为 `codex/g3-moss-transcript-20260813`；
- [G3 任务合同](G3-TASK-CONTRACT.md) 冻结了音频规范化、长音频分片、MOSS Adapter、HMAC 回调、回调/轮询恢复、真实分段时间（逐词可选）、合并校验和唯一 ACTIVE 原子发布；
...
- Fake MOSS 与未加载模型的 Provider Gateway 都不能代替真实模型推理；30 秒及 5/30/60/120 分钟样本未验证，G3 保持实施中而不关闭；
